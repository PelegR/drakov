...
var urlParser = require('./parse/url');
...
var isFormUrlEncoded = contentTypeChecker.isFormUrlEncoded;
...
function validateJson(reqBody, specBody, schema){
    ...
    // check that at least some of the keys from schema are present, but dont
    // fail if dont match exactly. Some keys can be optional and there is no
    // harm in passing extra body params than expected.
    if (specBody === null)
        return true;

    var keys = Object.keys(specBody);
    return keys.length === 0 || keys.some(function (specKey) {
        return typeof reqBody[specKey] !== 'undefined';
    });
...
function isBodyEqual( httpReq, specReq, contentType ) {
    ...
    var specBody = getBodyContent(specReq, isJson(contentType) || isFormUrlEncoded(contentType));
